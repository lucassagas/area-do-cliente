import { shade } from 'polished';

export const themes = {
  light: {
    background: '#FFFFFF',
    orange: '#FF4E00',
    orangeicons: '#e76126',
    opacityorange: 'rgba(240, 100, 40, 0.4)',
    text: '#212121',
    lighttext: '#ffff',
    icons: '#353535',
    lightgray: '#C8C8C8',
    error: '#c53030',
    purple: '#445495',
    green: '#44956A',
    greenopacity: 'rgba(69, 150, 107, 0.5)',
    darkgreen: '#3B6A52',
    backgroundcard: '#fafafa',
    backgroundmenu: '#cccc',
    whiteopacity: 'rgba(255, 255, 255, 0.5)',
    lightgreen: '#e6fffa',
    shimmerOne: '#e7edf1',
    shimmerTwo: '#f8f8f8',
    blueNotification: '#007BC2',
    redNotification: '#F02E2E',
    yellowNotification: '#ECB800',
    greenNotification: '#2FC448',
    darkgray: '#fff',
    backgroundNotification: '#F6F6F6',
    footer: '#212121',
    tooltipbg: '#FF824D',
  },

  dark: {
    background: '#212121',
    orange: '#FF4E00',
    orangeicons: '#e76126',
    opacityorange: 'rgba(240, 100, 40, 0.4)',
    text: '#FFFF',
    lighttext: '#ffff',
    icons: '#FFFF',
    lightgray: '#C8C8C8',
    error: '#c53030',
    purple: '#445495',
    green: '#44956A',
    greenopacity: 'rgba(69, 150, 107, 0.5)',
    darkgreen: '#3B6A52',
    backgroundcard: '#353535',
    backgroundmenu: '#cccc',
    whiteopacity: 'rgba(0, 0, 0, 0.5)',
    lightgreen: '#353535',
    shimmerOne: '#808080',
    shimmerTwo: '#BEBEBE',
    blueNotification: shade(0.1, '#007BC2'),
    redNotification: shade(0.2, '#F02E2E'),
    yellowNotification: shade(0.2, '#ECB800'),
    greenNotification: '#2FC448',
    darkgray: '#505050',
    backgroundNotification: '#353535',
    footer: '#353535',
    tooltipbg: '#63696E',
  },
};

export type ThemeName = keyof typeof themes;
export type ThemeType = typeof themes.light | typeof themes.dark;
